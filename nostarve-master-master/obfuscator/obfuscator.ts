...
async function obfuscate(code: string) {
    return new Promise(async(resolve, reject) => {
        ...
        for (let i = 0; i < Object.keys(SPRITE).length; i++) {
            ...
            const value: any = Object.values(SPRITE)[i]
        ...
        if (CONFIG.minify.object_properties) {
            for (const prop of CONFIG.object_properties) {
                ...
                if (regex.test(code)) replaceCode(regex, "." + char);
                if (regex_.test(code)) replaceCode(regex_, char + ":");
                if (regex__.test(code)) replaceCode(regex__, char + "(");
            }
        ...
        function replaceCode(pattern: RegExp, replaceValue: string) {
